(() => {
  ...
  const log = () => {};
  const mypageLogic = window.GlobisMypageLogic;
  ...
  const storageApi = () =>
    window.chrome && chrome.storage && chrome.storage.sync
      ? chrome.storage.sync
      : null;
  ...
  const loadSettings = () => {
    ...
    if (!api || !mypageLogic) {
      return Promise.resolve(mypageLogic ? mypageLogic.normalizeSettings() : {});
    }

    return new Promise((resolve) => {
      api.get(mypageLogic.SETTINGS_KEY, (result) => {
        const raw = result ? result[mypageLogic.SETTINGS_KEY] : undefined;
        resolve(mypageLogic.normalizeSettings(raw));
      });
    });
  };

  const normalize = (s) => String(s || "").replace(/\s+/g, " ").trim();
  const isTargetPage = () => location.pathname.startsWith("/my/");
  ...
  const onUrlChange = () => {
    ...
    if (!calendarEnabled || !isTargetPage()) return;
    startCalendarFeatures();
    scheduleDayDetailInjection();
  };
  ...
  const startCalendarFeatures = () => {
    if (!calendarEnabled) return;
    injectScript();
    startObserver();
    for (const delay of RUN_DELAYS_MS) {
      setTimeout(scheduleDayDetailInjection, delay);
    }
    log("calendar content script ready");
  };
